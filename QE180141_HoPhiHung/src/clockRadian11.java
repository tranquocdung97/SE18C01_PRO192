...
public class clockRadian11 {
    ...
    public static String clockRadian(String time) {
        ...
        double hAngle = 0.5 * (h * 60 + phut) *10 ;
        double phutAngle = 6 * phut *10 ;
        double angle = Math.abs(hAngle - phutAngle);
        angle = Math.min(360-angle, angle);
 

        // Rút gọn góc về dạng phân số
        int pso = ucln((int) angle, 3600);
        int tu = (int) (angle / pso);
        int mau = 3600 / pso;
...
//
//3. **Hàm greatestCommonDivisor**: Hàm này nhận vào hai số nguyên và trả về ước chung lớn nhất (UCLN) của chúng.
Đây là một thuật toán đệ quy cổ điển được sử dụng để rút gọn góc về dạng phân số trong hàm `clockRadian`.
//
